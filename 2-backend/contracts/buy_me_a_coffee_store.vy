...
@author GuireWire
@notice This contract is for the Full Stack Buy Me A Coffee Project on buymeacoffee.store
"""
...
def _fund_five_ineth():
    ...
    """
    usd_value_of_eth: uint256 = self._get_eth_to_usd_rate(
...
def _fund_three_inlink(amount: uint256):
    ...
    """
    usd_value_of_link: uint256 = self._get_link_to_usd_rate(
...
def get_eth_to_usd_rate(eth_amount: uint256) -> uint256:
    return self._get_eth_to_usd_rate(ETH_PRICE_FEED, eth_amount)

@external
@view
def get_link_to_usd_rate(link_amount: uint256) -> uint256:
    return self._get_link_to_usd_rate(LINK_PRICE_FEED, link_amount)
